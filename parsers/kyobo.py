...
def _parse_from_html(final_url: str, html: str, product_id: str | None) -> dict:
    ...
    # 셀렉터 보강(렌더링된 DOM에서 찾기)
    if sale_price is None:
        for sel in ["span.price","strong.price","em.price","span.prod_price","span.sale_price","div.price","span.total_price"]:
            el = s.select_one(sel)
            if el:
    ...
    if list_price is None:
        for sel in ["span.org_price","del.price","span.list_price","div.list_price","span.original_price"]:
            el = s.select_one(sel)
            if el:
