...
def _parse_from_html(final_url: str, html: str, product_id: str | None) -> dict:
    ...
    # extra selectors for tough sites
    if sale_price is None:
        for sel in ["span.price","strong.price","em.price","span.prod_price","span.sale_price","div.price"]:
            el = s.select_one(sel)
            if el:
                sale_price = parse_price(el.get_text(" ", strip=True))
                if sale_price: break
    if list_price is None:
        for sel in ["span.org_price","del.price","span.list_price","div.list_price"]:
            el = s.select_one(sel)
            if el:
                list_price = parse_price(el.get_text(" ", strip=True))
                if list_price: break

    status = "success" if (title or isbn) and (sale_price is not None or list_price is not None) else "failed"
    err = None if status == "success" else "가격 정보를 찾지 못했습니다(차단/동적 렌더링 가능)."

    return {"site": "KYobo", "url": final_url, "status": status, "product_id": product_id,
