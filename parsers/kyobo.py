...
def _parse_from_html(final_url: str, html: str, product_id: str | None) -> dict:
    ...
    return {
        ...
        "error": None if ((title or isbn) and (sale_price is not None or list_price is not None)) else "필수 정보를 찾지 못했습니다(차단/구조 변경 가능).",
    }
...
def parse_kyobo(url: str) -> dict:
    ...
    # requests 결과가 실패이거나, 가격이 의심스럽거나, 핵심 필드가 비면 playwright로 재시도
    needs_browser = (
        row["status"] != "success"
        or not row.get("title")
        or not row.get("isbn")
        or _is_suspicious(row.get("sale_price") or row.get("list_price"))
    )
    ...
    if needs_browser:
        try:
            ...
            elif (row2.get("title") or row2.get("isbn")):
                row2["status"] = "success"
                row2["error"] = None
            else:
                row2["status"] = "failed"
                row2["error"] = "필수 정보를 찾지 못했습니다(브라우저 렌더링 후에도 실패)."
            return row2
        except Exception as e:
            row["error"] = f"교보문고 브라우저 파싱 실패: {type(e).__name__}: {e}"
            return row
