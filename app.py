...
st.caption(
    "서점을 선택한 뒤, 웹 주소를 입력하고 도서 정보 가져오기 버튼을 클릭하면 ISBN/도서명/저자/출판사/가격이 자동으로 채워집니다. "
    "결과는 누적해 엑셀파일로 받아볼 수 있습니다."
)

with st.expander("✅ 지원 서점 / 사용 방법 / 참고사항", expanded=False):
    st.markdown(
        """
- 지원: **교보문고 / YES24 / 알라딘 / 영풍문고**
- 사용:
  1) 사용할 서점을 체크박스에서 선택  
  2) 상품 URL(웹 주소)을 입력(여러 줄 붙여넣기 가능)  
  3) **도서 정보 가져오기** → 테이블 누적  
  4) 누적 결과에서 **결과 엑셀(.xlsx) 다운로드**
- 참고사항:
...
def url_key(url: str) -> str:
    """중복 판정을 위한 URL 키 (트래킹 파라미터 제거 + 정규화)."""
    url = (url or "").strip()
    if not url:
        return ""
    try:
...
if run:
    ...
    if not urls:
        ...
        # --- 1) 기존 누적 결과에서 URL 기준 중복 정리 (마지막 1개만 남김) ---
        cleaned = []
        seen_keys = set()
        # 뒤에서부터 순회해서 마지막 항목을 우선 유지
        ...
        # --- 2) 이번에 파싱한 결과를 URL 기준으로 덮어쓰기(업데이트) / 신규는 추가 ---
        updated = 0
        appended = 0
        for r in new_rows:
            k = url_key(r.get("url", ""))
        ...
        # --- 3) ISBN 중복은 참고용으로만 표시(같은 URL 업데이트는 제외) ---
        isbn_seen = {}
        for i, row in enumerate(st.session_state.rows):
            ...
            if isbn in isbn_seen and url_key(st.session_state.rows[isbn_seen[isbn]].get("url","")) != url_key(row.get("url","")):
                # 이미 다른 URL(다른 서점 등)에서 같은 ISBN이 존재
                if not row.get("note"):
                    row["note"] = "⚠ 이미 추가된 도서"
            else:
                isbn_seen[isbn] = i

        st.success(f"{len(new_rows)}개 URL을 처리했어요. (신규 {appended}개 추가 / {updated}개 업데이트)")
